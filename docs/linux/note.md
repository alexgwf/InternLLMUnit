...
当前使用开发机为InternStduio。其相关介绍为(https://aicarrier.feishu.cn/wiki/GQ1Qwxb3UiQuewk8BVLcuyiEnHe)


1. 创建**开发机**，以及修改开发机配置和查看相关日志等。

![创建开发机](Img/01CreateDev.jpg)

点击“**创建开发机**”，选择创建**个人开发机**，名称为**dev00**，**Cuda**版本为12.2，**资源配置**选择10%，时长默认就行。

创建完成以后在**开发机**界面可以看到刚刚创建的开发机，点击进入开发机。

![创建好的开发机](Img/02Dev00.jpg)

进入开发机以后可以看到开发机的主页面，开发机有三种模式可以选择：**JupyterLab、终端和VScode**，下面节目为**Terminal**界面

![开发机Terminal界面](Img/03Dev00Terminal.jpg)

其中：

1. **JupyterLab**：一个交互式的编程和教学环境，同时内置终端，可以很方便地查看文件，执行代码等
1. **终端**（Terminal， 最轻量级）：主要进行命令行操作，或者运行脚本和简单程序
1. **VSCode**：网页中集成的VSCode，也可以在本地VSCode中通过SSH连接远程开发，下面就会讲如何配置远程连接。

## 2. SSH及端口映射

了解什么是**SSH**、**为什么使用远程连接**、如何使用SSH**远程连接**开发机、什么是**端口映射**以及如何进行**端口映射**。
...
将复制的命令粘贴到Terminal中，然后回车，输入密码。回车会出现以下内容：

![Local通过SSH登陆开发机成功界面](Img/04SSHTerminal.png)

当我们连接上开发机以后，可以使用`hostname`查看开发机名称，使用`uname -a`查看开发机内核信息，使用`lsb_release -a`查看开发机版本信息，使用`nvidia-smi`查看GPU的信息，想要退出远程连接，输入两次`exit`。

![上述命令演示](Img/05DevCommLine.png)

#### 2.2.2 配置SSH密钥进行SSH远程连接（可选）

但是在开发学习的时候，每次远程都输入密码比较麻烦，可以设置SSH key来跳过输入密码这一步骤，可以使用**ssh-keygen**命令来生成密钥
...
然后在开发机平台，在首页点击配置**SSH Key**，接着点击**添加SSH公钥**，
...
将生成的密钥复制下来，粘贴到公钥框中，名称会被自动识别到（**可能会不识别，自填写就好**），最后点击立即添加，SSH Key就配置完成了。

![image](https://github.com/InternLM/Tutorial/assets/110531742/79c3d51b-8cdf-4fc8-a5a3-1df5eab31933)

完成SSH Key创建以后，重启**终端**进行远程连接，就会跳过密码输入，直接进入。
...
![在开发机中创建hello_world.py](Img/07HelloWorld.png)

![安装Gradio](Img/08InsallGradio.png)

![复制上述代码](Img/09CopyCode.png)

![运行代码](Img/10RunCode.png)
...
![使用VSCode映射开发机端口](Img/12VSMapPort.png)
...
我们可以使用touch快速的创建文件，这样我们不用手动点击进行创建了。例如我们要创建一个`demo.py`文件，参考下图

#### 3.1.2 **mkdir**

同样的使用方法，如果要创建一个名为`test`的目录：

![Touch&Mkdir命令展示](Img/13TouchMkdir.png)
...
我们可以使用`pwd`命令查看当前所在的目录：这样可以方便我们确定我们当前所在哪个目录下面。查看下图。
...
![Pwd、Vim & Cat命令展示](Img/14PwdCatVim.png)
...
![Sed & Grep命令展示](Img/16SedGrep.png)

在示例中使用了`echo`命令，这和python中的`print`一样，用来打印内容，这里使用管道符`>`将InternLM打印到file文件中，常用的管道符还有`<`和`|`，比如我们可以使用`grep`命令来查看python中安装的包含`os`字段的包，参考上图。
